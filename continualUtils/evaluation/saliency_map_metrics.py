...
class SaliencyMapSamplePlugin(PluginMetric):
    ...
    def _make_grid_sample(
        ...
        rescaled_images = self._rescale_batch(self.images).cpu()
        rescaled_maps = self._rescale_batch(self.computed_maps).cpu()
    ...
    def _rescale_batch(self, inputs: Union[List[Tensor], Tensor]) -> Tensor:
